VEMmulti <- function(Smat, K, niter=100, epsilon_tau=1e-4, epsilon_eta=1e-4, verbose=FALSE, explorFact=1.5){
   ...
   N = nrow(Smat); d = ncol(Smat)
   n <-(1+sqrt(1+8*N))/2
   transfo_indices <- indices(n)
   mat_S <- Smat
   array_S <- array(dim=c(d, n, n)); 
   sapply(1:d, function(h){array_S[h, , ] <<- vect_mat_low(Smat[, h])})
   ...
   phi0 <- phi1 <- list(mu=rep(0, d), Sigma=matrix(0, d, d))
   ...
   while ((t < niter) & (diff>epsilon_tau)){
    ...
    #calcul de mu
    phi1$mu <- as.vector(Psi1_hat %*% mat_S / sum(Psi1_hat))
    phi0$mu <- as.vector(Psi0_hat %*% mat_S / sum(Psi0_hat))
    phi1$Sigma <- t(mat_S) %*% diag(Psi1_hat) %*% mat_S / sum(Psi1_hat) - phi1$mu%o%phi1$mu
    phi0$Sigma <- t(mat_S) %*% diag(Psi0_hat) %*% mat_S / sum(Psi0_hat) - phi0$mu%o%phi0$mu

    #calcul de fu:densité des 
    lfu <- cbind(dmvnorm(mat_S, phi0$mu, phi0$Sigma, log = TRUE), dmvnorm(mat_S, phi1$mu, phi1$Sigma, log = TRUE))
    dlfu = lfu[, 1] - lfu[, 2]
