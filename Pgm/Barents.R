...
library(sna); library(huge); library(blockmodels); library(EMtree); library(mclust); library(PLNmodels)
...
plnnet$plot()
plnnetOpt = plnnet$getBestModel()
Omega = plnnetOpt$model_par$Omega
Sigma = plnnetOpt$model_par$Sigma
Beta = t(plnnetOpt$model_par$Theta); d = nrow(Beta)

# Scores BM
# scoreGlasso = fitHuge(Sigma, method='glasso'); hist(scoreGlasso, breaks=p)
# vemGlasso = list(); for(K in 1:Kmax){vemGlasso[[K]] = VEM(S=scoreGlasso, K)}
# save(scoreGlasso, vemGlasso, file=paste0(dataDir, dataName, '-vemGlasso.Rdata'))
load(paste0(dataDir, dataName, '-vemGlasso.Rdata'))
...
# Choose K
lbGlasso = unlist(lapply(vemGlasso, function(v){v$borne_inf[length(v$borne_inf)]}))
lbTree = unlist(lapply(vemTree, function(v){v$borne_inf[length(v$borne_inf)]}))
plot(lbGlasso, type='b'); plot(lbTree, type='b')
