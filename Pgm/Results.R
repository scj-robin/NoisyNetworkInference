...
# Dims
p = 50; n = 100; K = 3; g = 2; gLassoMethod = 'mb'
simName = paste0('dataSimVEM-n', n, '-p', p, '-K', K, '-g', g, '-', gLassoMethod)

# Resultats
load(file=paste0(simDir, simName, '.Rdata'))
nbSim = length(simRes)
ARI = matrix(0, nbSim, 4); colnames(ARI) = c('Oracle', 'Glasso', 'logGlasso', 'Tree')
AUC = matrix(0, nbSim, 5); colnames(AUC) = c('Glasso', 'vemGlasso', 'vemLogGlasso', 'Tree', 'vemTree')
for(sim in 1:nbSim){
   cat(sim, '')
   # ARI
   ...
   AUC[sim, ] = c(
      performance(prediction(mat_vect_low(simRes[[sim]]$scoreGlasso), Gvec), "auc")@y.values[1][[1]],
